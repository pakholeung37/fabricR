...
const Image = createClass(
  ...
  /** @lends Image.prototype */ {
     ...
     * state of an object is changed ({@link Object#hasStateChanged})
     * as well as for history (undo/redo) purposes
     * @type Array
     */
    stateProperties: Object.prototype.stateProperties.concat("cropX", "cropY"),
    ...
    initialize: function (element, options) {
      options || (options = {})
      this.filters = []
      this.cacheKey = "texture" + Object.__uid++
    ...
    _renderDashedStroke: function (ctx) {
      ...
      ctx.beginPath()
      drawDashedLine(ctx, x, y, x + w, y, this.strokeDashArray)
      drawDashedLine(ctx, x + w, y, x + w, y + h, this.strokeDashArray)
      drawDashedLine(ctx, x + w, y + h, x, y + h, this.strokeDashArray)
      drawDashedLine(ctx, x, y + h, x, y, this.strokeDashArray)
      ctx.closePath()
      ctx.restore()
    },
    ...
    _render: function (ctx) {
      setImageSmoothing(ctx, this.imageSmoothing)
    ...
    drawCacheOnCanvas: function (ctx) {
      setImageSmoothing(ctx, this.imageSmoothing)
      Object.prototype.drawCacheOnCanvas.call(this, ctx)
    },
    ...
    _initElement: function (element, options) {
      this.setElement(getById(element), options)
      addClass(this.getElement(), Image.CSS_CANVAS)
    },
    ...
    parsePreserveAspectRatioAttribute: function () {
      var pAR = parsePreserveAspectRatioAttribute(
 ...
 */
Image.fromObject = function (_object, callback) {
  ...
  loadImage(
    ...
    function (img, isError) {
      ...
      Image.prototype._initFilters.call(object, object.filters, function (
        filters
      ) {
        object.filters = filters || []
        Image.prototype._initFilters.call(
          object,
          [object.resizeFilter],
          function (resizeFilters) {
            object.resizeFilter = resizeFilters[0]
            enlivenObjects([object.clipPath], function (enlivedProps) {
              object.clipPath = enlivedProps[0]
              var image = new Image(img, object)
...
Image.fromURL = function (url, callback, imgOptions) {
  loadImage(
    url,
    function (img, isError) {
      callback && callback(new Image(img, imgOptions), isError)
    },
    null,
 ...
 */
Image.ATTRIBUTE_NAMES = fabric.SHARED_ATTRIBUTES.concat(
...
Image.fromElement = function (element, callback, options) {
  ...
  Image.fromURL(
    ...
    callback,
    extend(options ? clone(options) : {}, parsedAttributes)
  )
}
/* _FROM_SVG_END_ */
