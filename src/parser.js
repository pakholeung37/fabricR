...
function _setStrokeFillOpacity(attributes) {
  for (var attr in colorAttributes) {
    ...
    if (typeof attributes[attr] === "undefined") {
      if (!FaObject.prototype[attr]) {
        continue
      }
      attributes[attr] = FaObject.prototype[attr]
    }
...
function parseUseDirectives(doc) {
  ...
  while (nodelist.length && i < nodelist.length) {
    var el = nodelist[i],
      xlink = (el.getAttribute("xlink:href") || el.getAttribute("href")).substr(
        1
      ),
 ...
 */
export const parseSVGDocument = function (
  doc,
  callback,
  reviver,
  parsingOptions
