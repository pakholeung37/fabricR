...
export function segmentToBezier(
  th2,
  th3,
  cosTh,
  sinTh,
 ...
 */
export function fromArcToBeizers(fx, fy, coords) {
  var rx = coords[1],
    ry = coords[2],
    rot = coords[3],
    large = coords[4],
...
export function makePathSimpler(path) {
...
export function getPathSegmentsInfo(path) {
  var totalLength = 0,
    len = path.length,
    current,
...
export function getPointOnPath(path, perc, infos) {
  if (!infos) {
