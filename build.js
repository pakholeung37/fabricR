...
var modulesToInclude = buildArgsAsObject.modules
  ? buildArgsAsObject.modules.split(",")
  : []
var modulesToExclude = buildArgsAsObject.exclude
  ? buildArgsAsObject.exclude.split(",")
  : []
...
if (amdLib === "requirejs" && minifier !== "uglifyjs") {
  console.log(
    "[notice]: require.js support requires uglifyjs as minifier; changed minifier to uglifyjs."
  )
  minifier = "uglifyjs"
  amdUglifyFlags =
    " -r 'require,exports,window,fabric' -e window:window,undefined "
}
...
if (sourceMap) {
  if (minifier !== "uglifyjs" && minifier !== "closure") {
    console.log(
      "[notice]: sourceMap support requires uglifyjs or google closure compiler as minifier; changed minifier to uglifyjs."
    )
    minifier = "uglifyjs"
  }
  sourceMapFlags =
    minifier === "uglifyjs"
      ? " --source-map fabric.min.js.map"
      : " --create_source_map fabric.min.js.map --source_map_format=V3"
}
...
if (minifier === "yui") {
  mininfierCmd =
    "java -jar " +
    rootPath +
    "/lib/yuicompressor-2.4.6.jar fabric.js -o fabric.min.js"
} else if (minifier === "closure") {
  mininfierCmd =
    "java -jar " +
    rootPath +
    "/lib/google_closure_compiler.jar --js fabric.js --js_output_file fabric.min.js" +
    sourceMapFlags
} else if (minifier === "uglifyjs") {
  mininfierCmd =
    "uglifyjs " +
    amdUglifyFlags +
    " --compress --mangle --output fabric.min.js fabric.js" +
    sourceMapFlags
}

var buildMinified = "build-minified" in buildArgsAsObject

var includeAllModules =
  (modulesToInclude.length === 1 && modulesToInclude[0] === "ALL") ||
  buildMinified

var noSVGImport =
  (modulesToInclude.indexOf("parser") === -1 && !includeAllModules) ||
...
function appendFileContents(fileNames, callback) {
  ;(function readNextFile() {
    ...
    fs.readFile(__dirname + "/" + fileName, function (err, data) {
      ...
      if (noSVGExport) {
        strData = strData.replace(
          /\/\* _TO_SVG_START_ \*\/[\s\S]*?\/\* _TO_SVG_END_ \*\//g,
          ""
        )
      }
      if (noSVGImport) {
        strData = strData.replace(
          /\/\* _FROM_SVG_START_ \*\/[\s\S]*?\/\* _FROM_SVG_END_ \*\//g,
          ""
        )
      }
      distFileContents += "\n" + strData + "\n"
...
function ifSpecifiedInclude(moduleName, fileName) {
  ...
  return (isInIncludedList || includeAllModules) && !isInExcludedList
    ? fileName
    : ""
}
    ...
    fs.writeFile("fabric.js", distFileContents, function (err) {
      ...
      exec(mininfierCmd, function (error, output) {
        ...
        console.log(
          "Minified using",
          minifier,
          "to " + distributionPath + "fabric.min.js"
        )

        if (sourceMapFlags) {
          console.log(
            "Built sourceMap to " + distributionPath + "fabric.min.js.map"
          )
        }

        exec("gzip -c fabric.min.js > fabric.min.js.gz", function () {
