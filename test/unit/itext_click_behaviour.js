  ...
  QUnit.module("iText click interaction", function (hooks) {
    ...
    QUnit.test("doubleClickHandler", function (assert) {
      ...
      assert.equal(
        ...
        20,
        "second dblClcik selection start is"
      )
    ...
    QUnit.test("tripleClickHandler", function (assert) {
      ...
      assert.equal(
        iText.selectionStart,
        20,
        "second tripleClick selection start is"
      )
      assert.equal(
        iText.selectionEnd,
        31,
        "second tripleClick selection end is"
      )
      iText.exitEditing()
    })
    QUnit.test("tripleClickHandler", function (assert) {
    ...
    QUnit.test("_getNewSelectionStartFromOffset end of line", function (
      assert
    ) {
      var iText = new fabric.IText("test need some word\nsecond line")
      var index = 10
    ...
    QUnit.test("_getNewSelectionStartFromOffset middle of line", function (
      assert
    ) {
      var iText = new fabric.IText("test need some word\nsecond line")
      var index = 10
      ...
      assert.equal(
        selection,
        index + 1,
        "index value was moved to next char, since is very near"
      )
    })
    QUnit.test("_getNewSelectionStartFromOffset middle of line", function (
      assert
      ...
      assert.equal(
        selection,
        index,
        "index value was NOT moved to next char, since is very near to first one"
      )
    })
    QUnit.test("_getNewSelectionStartFromOffset middle of line", function (
      assert
      ...
      assert.equal(
        selection,
        index,
        "index value was NOT moved to next char, since is already at end of text"
      )
    })
    QUnit.test("_mouseDownHandlerBefore set up selected property", function (
      assert
    ) {
      var iText = new fabric.IText("test need some word\nsecond line")
    ...
    QUnit.test("_mouseUpHandler on a selected object enter edit", function (
      assert
    ) {
      var iText = new fabric.IText("test")
      iText.initDelayedCursor = function () {}
        ...
        QUnit.test(
          ...
          function (assert) {
            ...
            assert.equal(
              iText.selectionEnd,
              0,
              "Itext did not set the selectionend"
            )
            ...
            setTimeout(function () {
              ...
              assert.equal(
                countCanvas,
                1,
                "no text:selection:changed fired yet"
              )
              done()
            }, 500)
          }
