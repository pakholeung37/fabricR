...
describe("fabric.Path", () => {
  ...
  test("initialize", function (done) {
    ...
    expect(path.left).toBe(100)
    expect(path.top).toBe(0)
    done()
  })

  test("initialize with strokeWidth", function (done) {
    var path = new fabric.Path("M 100 100 L 200 100 L 170 200 z", {
      strokeWidth: 50
    })

    expect(path.left).toBe(75)
    expect(path.top).toBe(75)
    done()
  })

  test("initialize with strokeWidth with originX and originY", function (done) {
    var path = new fabric.Path("M 100 100 L 200 100 L 170 200 z", {
      strokeWidth: 0,
      originX: "center",
      originY: "center"
    ...
    expect(path.left).toBe(150)
    expect(path.top).toBe(150)
    done()
  })
  ...
  test("toString", function (done) {
    makePathObject(function (path) {
      ...
      expect(path.toString()).toBe(
        '#<fabric.Path (4): { "top": 100, "left": 100 }>'
      )
      done()
    })
  })
  ...
  test("toObject", function (done) {
    makePathObject(function (path) {
      ...
      expect(obj.top).toBe(fabric.Object.prototype.top)
      expect(obj.left).toBe(fabric.Object.prototype.left)
      done()
    })
  })
  ...
  test("toSVG with a clipPath path", function (done) {
    makePathObject(function (path) {
  ...
  test("toSVG with a clipPath path absolutePositioned", function (done) {
    makePathObject(function (path) {
      makePathObject(function (path2) {
        path.clipPath = path2
  ...
  test("path array not shared when cloned", function (done) {
    makePathObject(function (originalPath) {
      originalPath.clone(function (clonedPath) {
        clonedPath.path[0][1] = 200
        expect(originalPath.path[0][1]).toBe(100)
        done()
      })
    })
  })

  test("toDatalessObject", function (done) {
    makePathObject(function (path) {
  ...
  test("toDatalessObject with sourcePath", function (done) {
    makePathObject(function (path) {
  ...
  test("fromObject with sourcePath", function (done) {
    expect(typeof fabric.Path.fromObject === "function").toBeTruthy()
    fabric.Path.fromObject(REFERENCE_PATH_OBJECT, function (path) {
      expect(path instanceof fabric.Path).toBeTruthy()
      expect(path.toObject()).toEqual(REFERENCE_PATH_OBJECT)
  ...
  test("fromElement", function (done) {
    ...
    fabric.Path.fromElement(elPath, function (path) {
      ...
      expect(path.toObject()).toEqual(
        fabric.util.object.extend(REFERENCE_PATH_OBJECT, {
          strokeDashArray: [5, 2],
          strokeLineCap: "round",
...
test("numbers with leading decimal point", function (done) {
  expect(typeof fabric.Path.fromElement === "function").toBeTruthy()
  var namespace = "http://www.w3.org/2000/svg"
  var elPath = fabric.document.createElementNS(namespace, "path")
...
test("multiple sequences in path commands", function (done) {
...
test("multiple M/m coordinates converted all L", function (done) {
...
test("multiple M/m commands converted all as M commands", function (done) {
...
test("compressed path commands", function (done) {
  var el = getPathElement(
...
test("compressed path commands with e^x", function (done) {
  var el = getPathElement(
