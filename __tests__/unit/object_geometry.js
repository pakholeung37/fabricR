...
describe("fabric.ObjectGeometry", () => {
  ...
  test("intersectsWithObject", function () {
    ...
    expect(object.intersectsWithObject(object1)).toBe(true)
    expect(object.intersectsWithObject(object2)).toBe(true)
    expect(object.intersectsWithObject(object3)).toBe(false)
  })
  ...
  test("isContainedWithinObject", function () {
    ...
    expect(object1.isContainedWithinObject(object)).toBe(true)
    expect(object2.isContainedWithinObject(object)).toBe(false)
    expect(object3.isContainedWithinObject(object)).toBe(false)
    object1.angle = 45
    object1.setCoords()
    expect(object1.isContainedWithinObject(object)).toBe(false)
    ...
    rect1.setCoords()
    rect2.setCoords()
    expect(rect1.isContainedWithinObject(rect2)).toBe(false)
  })
  ...
  test("containsPoint with padding", function () {
    ...
    expect(object.containsPoint(point2)).toBe(true)
    // point2 is outside of object (right)
    expect(object.containsPoint(point3)).toBe(false)
    // point3 is outside of object (bottom)
    expect(object.containsPoint(point4)).toBe(false)
    ...
    expect(object.containsPoint(point5)).toBe(false)
    // point5 is outside of object (top)
    expect(object.containsPoint(point6)).toBe(false)
  })
  ...
  test("setCoords", function () {
    ...
    expect(cObj.setCoords()).toBe(cObj)
    ...
    cObj.setCoords()
    // coords should still correspond to initial one, even after invoking `set`
    expect(cObj.oCoords.tl.x).toBe(225)
    expect(cObj.oCoords.tl.y).toBe(225)
    expect(cObj.oCoords.tr.x).toBe(375)
    expect(cObj.oCoords.tr.y).toBe(225)
    expect(cObj.oCoords.bl.x).toBe(225)
    expect(cObj.oCoords.bl.y).toBe(375)
    expect(cObj.oCoords.br.x).toBe(375)
    expect(cObj.oCoords.br.y).toBe(375)
    expect(cObj.oCoords.mtr.x).toBe(300)
    expect(cObj.oCoords.mtr.y).toBe(185)
  })
  ...
  test("setCoords and aCoords", function () {
    ...
    expect(cObj.oCoords.tl.x).toBe(300)
    expect(cObj.oCoords.tl.y).toBe(300)
    expect(cObj.oCoords.tr.x).toBe(500)
    expect(cObj.oCoords.tr.y).toBe(300)
    expect(cObj.oCoords.bl.x).toBe(300)
    expect(cObj.oCoords.bl.y).toBe(500)
    expect(cObj.oCoords.br.x).toBe(500)
    expect(cObj.oCoords.br.y).toBe(500)
    expect(cObj.oCoords.mtr.x).toBe(400)
    expect(cObj.oCoords.mtr.y).toBe(260)

    expect(cObj.aCoords.tl.x).toBe(150)
    expect(cObj.aCoords.tl.y).toBe(150)
    expect(cObj.aCoords.tr.x).toBe(250)
    expect(cObj.aCoords.tr.y).toBe(150)
    expect(cObj.aCoords.bl.x).toBe(150)
    expect(cObj.aCoords.bl.y).toBe(250)
    expect(cObj.aCoords.br.x).toBe(250)
    expect(cObj.aCoords.br.y).toBe(250)
  })
  ...
  test("transformMatrixKey depends from properties", function () {
    var cObj = new fabric.Object({
      left: -10,
      top: -10,
      width: 30,
  ...
  test("calcTransformMatrix with no group", function () {
  ...
  test("_calcDimensionsTransformMatrix with flipping", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 0,
  ...
  test("_calcTranslateMatrix", function () {
    var cObj = new fabric.Object({
      top: 5,
      width: 10,
      height: 15,
  ...
  test("scaleToWidth", function () {
    ...
    expect(cObj.scaleToWidth(100)).toBe(cObj)
    expect(cObj.getScaledWidth()).toBe(100)
    expect(cObj.get("scaleX")).toBe(100 / 560)
  })

  test("scaleToWidth with zoom", function () {
    var cObj = new fabric.Object({ width: 560, strokeWidth: 0 })
    cObj.canvas = {
      viewportTransform: [2, 0, 0, 2, 0, 0]
    }
    expect(cObj.scaleToWidth(100, true)).toBe(cObj)
    expect(cObj.getScaledWidth()).toBe(100)
    expect(cObj.get("scaleX")).toBe(100 / 560)
    expect(cObj.scaleToWidth(100)).toBe(cObj)
    expect(cObj.getScaledWidth()).toBe(50)
    expect(cObj.get("scaleX")).toBe(100 / 560 / 2)
  })
  ...
  test("scaleToHeight", function () {
    ...
    expect(cObj.scaleToHeight(100)).toBe(cObj)
    expect(cObj.getScaledHeight()).toBe(100)
    expect(cObj.get("scaleY")).toBe(100 / 560)
  })

  test("scaleToHeight with zoom", function () {
    var cObj = new fabric.Object({ height: 560, strokeWidth: 0 })
    cObj.canvas = {
      viewportTransform: [2, 0, 0, 2, 0, 0]
    }
    expect(cObj.scaleToHeight(100, true)).toBe(cObj)
    expect(cObj.getScaledHeight()).toBe(100)
    expect(cObj.get("scaleY")).toBe(100 / 560)
    expect(cObj.scaleToHeight(100)).toBe(cObj)
    expect(cObj.getScaledHeight()).toBe(50)
    expect(cObj.get("scaleY")).toBe(100 / 560 / 2)
  })
  ...
  test("getBoundingRect with absolute coords", function () {
    var cObj = new fabric.Object({
        strokeWidth: 0,
        width: 10,
        height: 10,
    ...
    expect(boundingRect.width).toBe(10)
    expect(boundingRect.height).toBe(10)
    cObj.canvas = {
      viewportTransform: [2, 0, 0, 2, 0, 0]
    }
    cObj.setCoords()
    ...
    expect(boundingRect.width).toBe(20)
    expect(boundingRect.height).toBe(20)
  ...
  test("getBoundingRect", function () {
    ...
    boundingRect = cObj.getBoundingRect()
    expect(boundingRect.left).toBe(0)
    expect(Math.abs(boundingRect.top).toFixed(13)).toBe("0.0000000000000")
    expect(boundingRect.width).toBe(123)
  ...
  test("getBoundingRectWithStroke", function () {
    ...
    cObj.setCoords()
    boundingRect = cObj.getBoundingRect()
    expect(boundingRect.left.toFixed(2)).toBe("0.00")
    expect(boundingRect.top.toFixed(2)).toBe("0.00")
    expect(boundingRect.width.toFixed(2)).toBe("1.00")
    expect(boundingRect.height.toFixed(2)).toBe("1.00")

    cObj.set("width", 123).setCoords()
    boundingRect = cObj.getBoundingRect()
    expect(boundingRect.left.toFixed(2)).toBe("0.00")
    expect(boundingRect.top.toFixed(2)).toBe("0.00")
    expect(boundingRect.width.toFixed(2)).toBe("124.00")
    expect(boundingRect.height.toFixed(2)).toBe("1.00")
    ...
    expect(boundingRect.top.toFixed(2)).toBe("0.00")
    expect(boundingRect.width.toFixed(2)).toBe("124.00")
    expect(boundingRect.height.toFixed(2)).toBe("168.00")

    cObj.scale(2).setCoords()
    boundingRect = cObj.getBoundingRect()
    expect(boundingRect.left.toFixed(2)).toBe("0.00")
    expect(boundingRect.top.toFixed(2)).toBe("0.00")
    expect(boundingRect.width.toFixed(2)).toBe("248.00")
    expect(boundingRect.height.toFixed(2)).toBe("336.00")
  })

  test("getScaledWidth", function () {
    var cObj = new fabric.Object()
  ...
  test("getCoords absolute with angle", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("getCoords with angle", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("getCoords absolute with skewX", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("getCoords with skewX", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("getCoords absolute with skewY", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("getCoords with skewY", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("getCoords absolute with skewY skewX angle", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("getCoords with skewY skewX angle", function () {
    var cObj = new fabric.Object({
      width: 10,
      height: 15,
      strokeWidth: 2,
  ...
  test("isPartiallyOnScreen", function () {
    var cObj = new fabric.Object({
      left: 50,
      top: 50,
      width: 100,
    ...
    cObj.top = -60
    cObj.setCoords()
    expect(cObj.isPartiallyOnScreen(true)).toBe(true)
    cObj.left = -110
    cObj.top = -110
    cObj.setCoords()
    expect(cObj.isPartiallyOnScreen(true)).toBe(false)
    cObj.left = 50
    cObj.top = 50
    cObj.setCoords()
    expect(cObj.isPartiallyOnScreen(true)).toBe(false)
  ...
  test("isPartiallyOnScreen with object inside and outside of canvas", function () {
    var cObj = new fabric.Object({
      left: 5,
      top: 5,
      width: 100,
